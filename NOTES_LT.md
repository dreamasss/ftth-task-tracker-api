...
Request su JWT
-> get_current_user
-> backend gauna current_user.id
-> sites endpointas ieško site pagal id IR user_id
-> jei site priklauso current_user, grąžina duomenis
-> jei site priklauso kitam useriui, grąžina 404
```
...
Endpointai gauna prisijungusį userį per:

```python
current_user: User = Depends(get_current_user)
```

Tada, kai reikia gauti konkretų site, endpointas kviečia:

```python
get_user_site_or_404(db, site_id, current_user.id)
```

Šita funkcija ieško site ne tik pagal `site_id`, bet ir pagal `user_id`.

Bloga logika būtų:

```text
SELECT site WHERE id = site_id
```

Gera logika:

```text
SELECT site WHERE id = site_id AND user_id = current_user.id
```

Jei site priklauso kitam useriui, query jo neranda ir backend grąžina `404 Site not found`.

Tai apsaugo nuo svetimų sites peržiūros, keitimo ir trynimo.

Trumpai:

```text
Auth pasako, kas useris.
Ownership pasako, kokie duomenys priklauso tam useriui.
```
...
total = kiek iš viso yra sites pagal userį
```
...
### Events kodo logika trumpai

Event priklauso konkrečiam site per `site_id`.

Prieš kuriant arba listinant events, backend pirmiausia patikrina site ownership per `get_user_site_or_404`.

Tai reiškia:
- useris gali pridėti event tik prie savo site
- useris gali matyti tik savo site eventus
- svetimas site grąžina `404 Site not found`

Kai `PATCH /sites/{site_id}` pakeičia statusą, backend automatiškai sukuria `status_change` eventą.
...
```text
Ką testuoja?
Patikrina, kad user B negali pridėti evento prie user A site.
...
```text
Events endpointai irgi naudoja `get_user_site_or_404`.

Prieš listinant arba kuriant eventą, backend pirmiausia patikrina, ar site priklauso prisijungusiam useriui.

Trumpai:

`JWT -> current_user.id -> get_user_site_or_404(site_id, current_user.id) -> event logic`
...
```text
Ką testuoja?
Patikrina, ar visa aplikacija veikia per realius HTTP requestus: `/`, `/health`, register, login, sites, filters, sorting, events ir stats.
...
Bet galima paleisti ir prieš live API:
